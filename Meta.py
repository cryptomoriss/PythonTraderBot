...
def GetBrokerTimeStr(symbol="XAUUSD_o"):
    tick = mt5.symbol_info_tick(symbol)
    if tick is None:
        return datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S") + " (fallback: UTC, no tick)"
    return datetime.fromtimestamp(tick.time, tz=timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
...
class Meta:
    ...
    def WaitUntilMarketOpen(symbol, lot, buy, sell, ticket=None, pct_tp=0.02, pct_sl=0.01, magic=0, result=None, stopLossWithAtr=False, stopLossPure=False, comment="SP2L Bot"):
                ...
                if resume.shape[0] > 0:
                    try:
                        ...
                        if not row.empty:
                            ...
                            result = Meta.SendOrder(symbol, lot, buy, sell, ticket=ticket,pct_tp=pct_tp, pct_sl=pct_sl, comment=comment, magic=magic, stopLossWithAtr=stopLossWithAtr, stopLossPure=stopLossPure)
                        else:
    ...
    def run(symbol, buy, sell, lot, pct_tp=0.02, pct_sl=0.01, magic = 0, stopLossWithAtr=False, stopLossPure=False, comment="SP2L Bot"):
        ...
        if buy == True or sell ==True:
            ...
            # اگر سفارش خرید داشتی و الان باید ببندیش                
                ...
                before = mt5.account_info().balance
                result = Meta.SendOrder(symbol, lot, True, False, ticket=ticket,pct_tp=pct_tp, pct_sl=pct_sl, comment=comment, magic=magic, stopLossWithAtr=stopLossWithAtr, stopLossPure=stopLossPure)
                
                # درصورتی که بازار بسته بشه بایستی صبر کرد تا زمانی که بازار باز شود و سفارش را ببندیم
                if result.comment == "Market closed":
                    print("Market closed!")
                    marketClosed = True
                    result, passBecauseStopLossHit, before = Meta.WaitUntilMarketOpen(symbol, lot, True, False, ticket, pct_tp, pct_sl, magic, result, stopLossWithAtr=stopLossWithAtr, stopLossPure=stopLossPure, comment=comment)                
                ...
                if result != None:
                    ...
                    print("-"*75)
                    print("Date: ", GetBrokerTimeStr(symbol), "\tSYMBOL:", symbol)
                ...
                before = mt5.account_info().balance
                result = Meta.SendOrder(symbol, lot, False, True, ticket=ticket,pct_tp=pct_tp, pct_sl=pct_sl, comment=comment, magic=magic, stopLossWithAtr=stopLossWithAtr, stopLossPure=stopLossPure)

                # درصورتی که بازار بسته بشه بایستی صبر کرد تا زمانی که بازار باز شود و سفارش را ببندیم
                if result.comment == "Market closed":
                    print("Market closed!")
                    marketClosed = True
                    result, passBecauseStopLossHit, before = Meta.WaitUntilMarketOpen(symbol, lot, False, True, ticket, pct_tp, pct_sl, magic, result, stopLossWithAtr=stopLossWithAtr, stopLossPure=stopLossPure, comment=comment)                
                if (marketClosed and passBecauseStopLossHit == False):
                    print("Market open!")
                    marketClosed = False
                if passBecauseStopLossHit:
                ...
                result =  Meta.SendOrder(symbol, lot, True, False, ticket=None,pct_tp=pct_tp, pct_sl=pct_sl, comment=comment, magic=magic, stopLossWithAtr=stopLossWithAtr, stopLossPure=stopLossPure)
                ...
                if result != None:
                    ...
                    if result.comment == "Market closed":
                        ...
                        result, passBecauseStopLossHit, before = Meta.WaitUntilMarketOpen(symbol, lot, True, False, None, pct_tp, pct_sl, magic, result, stopLossWithAtr=stopLossWithAtr, stopLossPure=stopLossPure, comment=comment)                
                ...
                result = Meta.SendOrder(symbol, lot, False, True, ticket=None,pct_tp=pct_tp, pct_sl=pct_sl, comment=comment, magic=magic, stopLossWithAtr=stopLossWithAtr, stopLossPure=stopLossPure)
                ...
                if result != None:
                    ...
                    if result.comment == "Market closed":
                        ...
                        # در سفارش گذاری نیاز به دو متغیر بعد از ریزالت نیست
                        # فقط چون نخواستم دوباره تابع تعریف کنم گذاشتم بمونه
                        result, passBecauseStopLossHit, before = Meta.WaitUntilMarketOpen(symbol, lot, False, True, None, pct_tp, pct_sl, magic, result, stopLossWithAtr=stopLossWithAtr, stopLossPure=stopLossPure, comment=comment)                
